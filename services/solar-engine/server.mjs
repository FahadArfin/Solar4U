...
import { localEstimate, fitGroundArray, withProduction } from "./model.mjs";
...
async function roofVisionCapabilities() {
  ...
  try {
    const response = await fetch(
      `${roofVisionUrl()}/v1/roof-vision/capabilities`,
...
async function proxyRoofVision(request, response, requestId, pathname) {
  if (!roofVisionUrl())
    return (
      json(response, 503, { error: "roof_vision_not_configured" }, requestId) ||
      true
    );
...
async function getDataLayerManifest(latitude, longitude) {
  ...
  if (!upstream.ok)
    throw new Error(
      payload?.error?.message || `google_data_layers_${upstream.status}`,
    );
  dataLayerManifestCache.set(cacheKey, {
    payload,
...
async function fetchGoogleDataLayer(layerUrl) {
  ...
  const response = await fetch(
    `${layerUrl}${separator}key=${encodeURIComponent(googleSolarKey())}`,
    {
...
async function detectMaskedRoofPlanes(latitude, longitude) {
  ...
  form.append(
    "dsm",
    new Blob([dsm], { type: "image/tiff" }),
    "google-solar-dsm.tif",
  );
  form.append(
    "building_mask",
    new Blob([buildingMask], { type: "image/tiff" }),
    "google-solar-building-mask.tif",
  );
  ...
  form.append(
    "source_license",
    "Google Solar API Data Layers; selected building mask and DSM processed for the current analysis",
  );
  const upstream = await fetch(
    `${roofVisionUrl()}/v1/roof-vision/solar-planes`,
    {
      method: "POST",
      body: form,
      signal: AbortSignal.timeout(120000),
  ...
  if (!upstream.ok)
    throw new Error(
      payload?.detail || `roof_plane_detection_${upstream.status}`,
    );
  return {
    ...payload,
    data: {
...
async function pvgisEstimate(input) {
  ...
  if (monthly.length !== 12)
    throw new Error("PVGIS returned an incomplete monthly estimate");
  const base = localEstimate(input);
  const monthlyKwh = monthly.map((month) => Math.round(Number(month.E_m)));
  const annualKwh = monthlyKwh.reduce((sum, value) => sum + value, 0);
  return {
    ...withProduction(input, base, monthlyKwh, "eu-jrc-pvgis-5.3"),
    provider: "eu-jrc-pvgis-5.3",
    providerVersion: "5.3",
    annualKwh,
    monthlyKwh,
    yearlyBillValue: Math.round(
      annualKwh * Number(input.electricityRate ?? 0.19),
    ),
...
async function pvWatts(input) {
  if (Array.isArray(input.arrays) && input.arrays.length) {
    if (input.arrays.length > 12) throw new Error("Use at most twelve arrays");
    const estimates = await Promise.all(
      input.arrays.map((array) =>
        pvWatts({
          ...input,
          arrays: undefined,
          ...array,
    ...
    const monthlyKwh = Array.from({ length: 12 }, (_, month) =>
      estimates.reduce(
        (sum, estimate) => sum + Number(estimate.monthlyKwh?.[month] || 0),
        0,
      ),
    );
    const annualKwh = monthlyKwh.reduce((sum, value) => sum + value, 0);
    return {
      ...withProduction(
        {
          ...input,
          capacityKw: input.arrays.reduce(
            (sum, array) => sum + Number(array.capacityKw ?? 0),
            0,
          ),
        },
        localEstimate({
      ...
      provider: estimates.every(
        (estimate) => estimate.provider === "eu-jrc-pvgis-5.3",
      )
        ? "eu-jrc-pvgis-5.3-multi-array"
        : "solar4u-mixed-provider",
      annualKwh,
      monthlyKwh,
      yearlyBillValue: Math.round(
        annualKwh * Number(input.electricityRate ?? 0.19),
  ...
  if (input.provider === "pvgis") {
    try {
      ...
      return {
        ...localEstimate(input),
        warnings: [`Live PVGIS estimate unavailable: ${error.message}`],
      };
    }
  }
  ...
  const response = await fetch(
    `https://developer.nlr.gov/api/pvwatts/v8.json?${params}`,
  );
  ...
  return {
    ...withProduction(
      input,
      localEstimate(input),
      payload.outputs.ac_monthly.map(Math.round),
      "nrel-pvwatts-v8",
    ),
    providerVersion: payload.version,
...
serve({
  ...
  router: async (request, response, requestId) => {
    ...
    if (
      ...
      url.pathname.startsWith("/v1/solar/roof-vision/")
    ) {
    ...
    if (
      ...
      const downstreamPath = url.pathname.replace(
        "/v1/solar/roof-vision/",
        "/v1/roof-vision/",
      );
    ...
    if (
      request.method === "GET" &&
      url.pathname === "/v1/solar/photorealistic-3d-config"
    ) {
      ...
      if (!key)
        return (
          json(
            response,
            503,
            { error: "google_map_tiles_key_not_configured" },
            requestId,
          ) || true
        );
      return (
      ...
      if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
        return (
          json(
            response,
            400,
            { error: "valid_latitude_and_longitude_required" },
            requestId,
          ) || true
        );
      }
      ...
      if (!mapResponse.ok)
        return (
          json(
            response,
            mapResponse.status,
            { error: "google_static_map_unavailable" },
            requestId,
          ) || true
        );
      ...
      if (!googleSolarKey())
        return (
          json(
            response,
            503,
            { error: "google_solar_key_not_configured" },
            requestId,
          ) || true
        );
      if (!roofVisionUrl())
        return (
          json(
            response,
            503,
            { error: "roof_vision_not_configured" },
            requestId,
          ) || true
        );
    ...
    if (request.method === "GET" && url.pathname === "/v1/solar/data-layer") {
      ...
      const month = Math.max(
        0,
        Math.min(11, Number(url.searchParams.get("month") || 0)),
      );
      if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
        return (
          json(
        ...
        layer === "rgb"
          ...
          : layer === "mask"
            ...
            : layer === "dsm"
              ? manifest.dsmUrl
              : layer === "annualFlux"
                ? manifest.annualFluxUrl
                : layer === "monthlyFlux"
                  ? manifest.monthlyFluxUrl
                  : layer === "hourlyShade"
                    ? manifest.hourlyShadeUrls?.[month]
                    : "";
      if (!layerUrl)
        return (
          json(
            response,
            404,
            { error: "solar_data_layer_not_available" },
            requestId,
          ) || true
        );
      let body;
      try {
        ...
        return (
          json(
            response,
            502,
            { error: "solar_geotiff_unavailable" },
            requestId,
          ) || true
        );
      }
          ...
          if (upstream.ok && payload.status === "OK") {
            ...
            return (
              json(
                response,
                200,
                { data: results, attribution: "Geocoding by Google" },
                requestId,
              ) || true
            );
          }
      ...
      const upstream = await fetch(
        `https://nominatim.openstreetmap.org/search?${params}`,
        {
          headers: {
            ...
            "User-Agent":
              "Solar4U/0.1 local planning tool (address search initiated by user)",
          },
          signal: AbortSignal.timeout(9000),
        },
      );
      ...
      const results = (payload || [])
        .map((item) => {
          ...
          return {
            ...
            resultTypes: [item.class, item.type, item.addresstype].filter(
              Boolean,
            ),
        ...
        .sort((a, b) => {
          const score = (value) =>
            value.confidence === "street_address" ? 0 : 1;
      ...
      if (query.length < 2)
        return (
          json(
            response,
            400,
            { error: "location_query_too_short" },
            requestId,
          ) || true
        );
      const params = new URLSearchParams({
      ...
      return (
        json(
          ...
          upstream.ok ? 200 : upstream.status,
          {
            data: locations,
            attribution:
              "Location search by Open-Meteo; data based on GeoNames.",
          },
          requestId,
        ) || true
      );
    }
    if (request.method === "POST" && url.pathname === "/v1/solar/estimates")
      return (
        json(
          response,
          200,
          { data: await pvWatts(await readJson(request)) },
          requestId,
        ) || true
      );
    if (request.method === "POST" && url.pathname === "/v1/solar/ground-layout")
      return (
        json(
          response,
          200,
          { data: fitGroundArray(await readJson(request)) },
          requestId,
        ) || true
      );
    if (
      request.method === "POST" &&
      url.pathname === "/v1/solar/roof-analysis"
    ) {
          ...
          json(
            ...
            200,
            {
              data: {
                ...
                available: false,
                reason: "google_solar_key_not_configured",
      ...
      const params = new URLSearchParams({
        "location.latitude": String(body.latitude),
        "location.longitude": String(body.longitude),
        requiredQuality: body.requiredQuality || "HIGH",
    ...
    if (
      request.method === "POST" &&
      url.pathname.startsWith("/v1/calculators/")
    ) {
      ...
      return (
        json(
          response,
          calculatorHandlers[type] ? 200 : 404,
          calculatorHandlers[type]
            ? { data: calculatorHandlers[type](body) }
