...
export default function SolarBuilder() {
  ...
  async function addSelection(offer: Choice, quantity?: number) {
    ...
    <main className="s4-builder">
      <header className="s4-build-heading">
        <div>
          ...
          <p>
            Choose every part of your system. Compare the details. Keep your
            budget in view.
          </p>
        </div>
      ...
      {view === "build" ? (
        <div className="s4-build-layout">
          <section>
            <div className="s4-build-section-title">
              <div>
                <h2 ref={buildHeader} tabIndex={-1}>
                  Your component list
                </h2>
                <p>
                  Use the categories your design needs. You can add more than
                  one part to any group.
                </p>
              </div>
              <button
                onClick={() =>
            ...
            <div className="s4-build-slots">
              {shoppingGroups.map((g) => {
                const items = store.data.items.filter(
                    (i) => itemGroup(i) === g.id,
                  ),
                  Icon = icons[g.id];
                if (["bundles", "other"].includes(g.id) && !items.length)
                  return null;
                return (
                  <section className="s4-build-slot" key={g.id}>
                    <header>
                      <Icon size={22} />
                      <div>
                        <h3>{g.name}</h3>
                      ...
                      <button onClick={() => chooseGroup(g.id)}>
                        <Plus size={15} />{" "}
                        {items.length ? "Add more" : "Choose"}
                      </button>
                    </header>
                    {items.length ? (
                      <div className="s4-build-selected">
                        {items.map((i) => (
                          <div key={i.id}>
                            <div>
                              ...
                              <small>
                                {i.source
                                  ? `${i.source.retailer} · ${i.source.variant === "Default Title" ? i.source.sku || "Listed variant" : i.source.variant} · observed ${new Date(i.source.observedAt).toLocaleDateString()}`
                                  : "Manual item / allowance"}
                                {i.source?.imported
                                  ? " · imported snapshot"
                                  : ""}
                              </small>
                              <span>
                                {i.quantity} purchased units
                                {classifyOffer({
                                  name: i.name,
                                  variant: i.source?.variant,
                                }).specs.packCount
                                  ? ` · package contents need confirmation`
                                  : ""}
                              </span>
                            </div>
                            <div>
                      ...
                      <div className="s4-build-slot-options">
                        {g.subtypes
                          .filter(([s]) => s !== "unknown")
                          .map(([id, label]) => (
                            <button
                              key={id}
                              onClick={() => chooseGroup(g.id, id)}
                            >
                              {label}
            ...
            <div className="s4-build-extras">
              <button onClick={() => chooseGroup("bundles")}>
                Explore complete kits & power stations ↗
              </button>
              <Link href="/equipment">
                Add a manual item, quote or allowance ↗
              </Link>
            </div>
          </section>
          <aside className="s4-build-budget">
            <div className="s4-kicker">YOUR WORKING BUDGET</div>
            <h2>{store.data.name}</h2>
            {Object.entries(totals).length ? (
              Object.entries(totals).map(([c, t]) => (
                <div className="s4-build-total" key={c}>
                  <span>{c} known-price subtotal</span>
                  <strong>{formatMinor(t.minorAmount, c)}</strong>
                  <small>
                    {t.unknown
                      ? `${t.unknown} unpriced units excluded`
                      : "All listed units priced"}
                  </small>
                </div>
              ))
            ) : (
              <p>Add your first component to start a budget.</p>
            )}
            <dl>
              <div>
                <dt>Shipping & freight</dt>
                <dd>Needs retailer quote</dd>
              </div>
              <div>
                <dt>Taxes</dt>
                <dd>Not estimated</dd>
              ...
              <div>
                <dt>Different currencies</dt>
                <dd>Kept separate</dd>
              </div>
            </dl>
            <p>
              Prices are dated observations, not checkout totals. No purchase is
              made when you add a part.
            </p>
            <Link className="s4-button s4-dark" href="/equipment">
              Review your full equipment list <ArrowUpRight size={15} />
            </Link>
            <details open>
              <summary>Compatibility needs more information</summary>
              <p>
                Nominal voltage or matching wattage alone does not establish
                compatibility.
              </p>
              {[
                [
                  "Panel strings → MPPT",
                  "Cold Voc, hot Vmp, current limits and string layout",
                ],
                [
                  "Battery → inverter",
                  "Operating range, BMS support and charge limits",
                ],
                [
                  "Panels → mounting",
                  "Dimensions, clamp zones and approved hardware",
                ],
                [
                  "Protection & wiring",
                  "AC/DC ratings, fault current, cable length and installation conditions",
                ],
              ].map(([h, p]) => (
                <div className="s4-build-check" key={h}>
                  <b>{h}</b>
                  <span>{p}</span>
                    ...
                    <article key={o.id} className="s4-shopping-offer">
                      ...
                      <div className="s4-shopping-price">
                        ...
                        <button
                          ...
                          aria-label={`Add ${o.name}, ${o.variant}, from ${o.retailer} to build`}
                          disabled={
                            adding || !store.ready || store.needsRecovery
                          }
                          onClick={() => void addSelection(o)}
                        >
                          {inBuild.has(o.id) ? (
                            <Check size={15} />
                          ) : (
                          ...
                          {inBuild.has(o.id) ? "Add another" : "Add to build"}
                        </button>
                        <a href={o.url} target="_blank" rel="noreferrer">
                          Shop retailer ↗
                        </a>
