...
export function usePageTools(){
  ...
  useAgentTools([
  {name:"solar4u_read_page",title:"Read Solar4U page",description:"Read the current Solar4U page title, route, and available navigation. Does not expose local project contents or account data.",inputSchema:{type:"object",properties:{},additionalProperties:false},annotations:{readOnlyHint:true},execute:input=>{objectInput(input);return {title:document.title,path:location.pathname,sections:["/planner","/roof-analysis","/products","/guides","/calculators","/dashboard","/solar-part-picker","/equipment","/diagrams","/diagnostics"]}}},
  ...
  {name:"solar4u_start_navigation",title:"Open a Solar4U workspace",description:"Start navigation to a known Solar4U section. This opens the requested workspace; it does not create or save a project.",inputSchema:{type:"object",properties:{section:{type:"string",enum:["planner","roof-analysis","products","guides","calculators","dashboard","solar-part-picker","equipment","diagrams","diagnostics"]}},required:["section"],additionalProperties:false},annotations:{readOnlyHint:false},execute:input=>{const {section}=objectInput(input);if(typeof section!=="string"||!["planner","roof-analysis","products","guides","calculators","dashboard","solar-part-picker","equipment","diagrams","diagnostics"].includes(section))throw new Error("Unknown Solar4U section");window.setTimeout(()=>location.assign(`/${section}`),0);return {status:"navigation_requested",path:`/${section}`}}}
])}
