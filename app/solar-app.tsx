...
import SolarPartPicker, { ProductHubPage } from "./solar-part-picker";
...
export function SolarApp({ section, productId = "", pickerSearch = "", productReturnTo = "/catalog" }: { section: string; communityThreadId?: string; productId?: string; pickerSearch?: string; productReturnTo?: string }) {
  usePageTools();
  let content=<NewHome/>;
  if(["products","catalog","deals","recommendations"].includes(section)) content=<ProductHubPage/>;
  ...
  return <div className="s4-app"><a href="#main-content" className="s4-skip">Skip to content</a><SiteHeader section={section}/><div id="main-content" className={["home","dashboard","admin","diagnostics","community","community-test"].includes(section)?"":"legacy-app"}>{content}</div><SiteFooter/></div>;
}
