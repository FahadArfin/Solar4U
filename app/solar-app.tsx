...
function DiagramsPage() {
  const [nodes, setNodes] = useState<DiagramNode[]>([
    { id: 1, type: "PV array", x: 80, y: 100 },
    { id: 2, type: "Disconnect", x: 310, y: 100 },
    { id: 3, type: "Inverter", x: 540, y: 100 },
    { id: 4, type: "Battery", x: 540, y: 280 },
    { id: 5, type: "Main panel", x: 760, y: 100 },
  ]);
  const [selected, setSelected] = useState<number | null>(null);
  const add = (type: string) =>
    setNodes((n) => [
      ...n,
      { id: Date.now(), type, x: 100 + (n.length % 4) * 190, y: 380 },
    ]);
  return (
    <main className="diagram-page">
      <div className="diagram-toolbar">
        <div>
          <div className="eyebrow">ONE-LINE BUILDER</div>
      ...
      <div className="diagram-shell">
        <aside>
          ...
          {[
            ...
            <button key={x} onClick={() => add(x)}>
              <span>
                {x === "PV array" ? "▦" : x === "Battery" ? "▤" : "⌁"}
              </span>
              {x}
              <b>+</b>
            </button>
        ...
        <section className="canvas">
          ...
          <svg className="wires">
            <line x1="170" y1="135" x2="350" y2="135" />
            <line x1="390" y1="135" x2="575" y2="135" />
            <line x1="620" y1="135" x2="795" y2="135" />
            <line className="negative" x1="575" y1="180" x2="575" y2="285" />
          </svg>
          {nodes.map((n) => (
            <button
              key={n.id}
            ...
            >
              <span>
                {n.type === "PV array" ? "▦" : n.type === "Battery" ? "▤" : "⌁"}
              </span>
              <b>{n.type}</b>
              <small>
                {n.type === "PV array"
              ...
              <i className="port left"></i>
              <i className="port right"></i>
            </button>
          ))}
        </section>
        <aside className="inspector">
          ...
          <div className="validation good">
            <b>✓ 6 checks passed</b>
            <span>
              Voltage, polarity and controller input are within configured
              limits.
            </span>
          </div>
          <div className="validation warn">
            <b>△ Review conductor</b>
            <span>
              Specify wire length and temperature rating to validate voltage
              drop.
            </span>
          </div>
          ...
          <p className="disclaimer">
            Planning assistance only. A passing check is not engineering
            approval or proof of code compliance.
          </p>
        </aside>
      </div>
    </main>
...
export function SolarApp({
  ...
  if (["dashboard", "community", "community-test"].includes(section))
    content = <WorkspacePage />;
  if (["admin", "diagnostics"].includes(section)) content = <DiagnosticsPage />;
  return (
    <div className="s4-app">
      <a href="#main-content" className="s4-skip">
        Skip to content
      </a>
      <SiteHeader section={section} />
      <div
        id="main-content"
        className={section === "diagrams" ? "legacy-app" : ""}
      >
        {content}
      </div>
      <SiteFooter />
